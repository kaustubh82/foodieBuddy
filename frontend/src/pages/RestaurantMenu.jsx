                        ...
                        <div>
                            ...
                            <div className="flex items-center space-x-4 text-sm text-gray-400">
                                <div className="flex items-center space-x-1">
                                    ...
                                    <span className="font-medium text-gray-200">{restaurant.rating.toFixed(1)}</span>
                                </div>
                                <div className="flex items-center space-x-1">
                                    <MapPin size={18} />
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {menuItems.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {menuItems.map((item) => (
                            <div key={item.id} className="card p-4 hover:bg-gray-900/50 transition-colors">
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex-1">
                                        <h3 className="text-lg font-semibold mb-1 text-gray-100">{item.name}</h3>
                                    ...
                                    {item.imageUrl && (
                                        <img
                                            src={item.imageUrl}
                                            alt={item.name}
                                            className="w-20 h-20 object-cover rounded-lg ml-3 bg-gray-800"
                                        />
                                    )}
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-gray-300 bg-gray-800 px-2 py-1 rounded border border-gray-700">
                                        {item.category}
                                    </span>
                                    {item.isAvailable ? (
                                        <button
                                            onClick={() => handleAddToCart(item)}
                                            className="btn-primary py-1 px-3 text-sm flex items-center space-x-1 shadow-lg shadow-primary-900/20"
                                        >
                                            <Plus size={16} />
                                            <span>Add</span>
                                        </button>
